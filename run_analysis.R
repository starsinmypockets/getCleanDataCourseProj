...
features <- sapply(features, function (x) {gsub('[()-,]', "", x); }); # sanitize variable names
...
testActivity <- apply(testActivity, 1, addActivityNames); # update activity names
...
trainActivity <- apply(trainActivity, 1, addActivityNames); # update activity names
...
# get data frame with desired fields 
